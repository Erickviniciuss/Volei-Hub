...
function renderOverview() {
  ...
  overviewRounds.innerHTML = quickSchedule.map((round, index) => {
    ...
    const matches = round.matches.map(([home, away], gameIndex) => {
      ...
      if (editable) return `<div class="match overview-match retro-match"><span>${escapeQuick(home)}</span><span class="retro-score-inputs"><input data-retro-round="${index}" data-retro-game="${gameIndex}" data-side="0" type="number" min="0" inputmode="numeric" value="${result?.[0] ?? ""}" aria-label="Novo placar de ${escapeQuick(home)}" /><b>×</b><input data-retro-round="${index}" data-retro-game="${gameIndex}" data-side="1" type="number" min="0" inputmode="numeric" value="${result?.[1] ?? ""}" aria-label="Novo placar de ${escapeQuick(away)}" /></span><span class="team-away">${escapeQuick(away)}</span></div>`;
